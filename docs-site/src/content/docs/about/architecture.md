---
title: Architecture
description: How @jbcom/medieval-hexagon-gameboard is organized — 20 domain sub-packages, koota ECS runtime, three-layer build pipeline.
sidebar:
  order: 2
---

The library is one published npm package (`@jbcom/medieval-hexagon-gameboard`) that internally decomposes into 20 domain sub-packages under `src/`. Cross-domain imports traverse barrel re-exports only — Biome's `noRestrictedImports` rule enforces it.

## Sub-package map

| Sub-package | Purpose | Public surface |
|---|---|---|
| `traits/` | koota trait declarations (37 traits) | `@jbcom/medieval-hexagon-gameboard/traits` |
| `types/` | Branded primitives + interface definitions | `@jbcom/medieval-hexagon-gameboard/types` |
| `coordinates/` | Hex algebra, projection, grid layout, world↔hex transforms | `/coordinates`, `/grid`, `/layout`, `/projection` |
| `manifest/` | KayKit pack manifest schema + the autogenerated FREE manifest | `/manifest/schema`, `/manifest/free`, `/manifest/upstream-layout` |
| `ingest/` | Node-side asset ingest, source-root validation, manifest generation | `/ingest` |
| `bootstrap/` | CLI `bootstrap` subcommand + programmatic API; fetches + verifies KayKit assets | `/bootstrap` |
...
| `cli/` | The `medieval-hexagon-gameboard` Node binary | `/cli` |
...
- The actual GLTF tree is fetched at install time by `pnpm exec medieval-hexagon-gameboard bootstrap`.
