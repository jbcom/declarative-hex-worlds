---
title: Architecture
description: How medieval-hexagon-gameboard is organized — 20 domain sub-packages, koota ECS runtime, three-layer build pipeline.
sidebar:
  order: 2
---

The library is one published npm package (`medieval-hexagon-gameboard`) that internally decomposes into 20 domain sub-packages under `src/`. Cross-domain imports traverse barrel re-exports only — Biome's `noRestrictedImports` rule enforces it.

## Sub-package map

| Sub-package | Purpose | Public surface |
|---|---|---|
| `traits/` | koota trait declarations (37 traits) | `medieval-hexagon-gameboard/traits` |
| `types/` | Branded primitives + interface definitions | `medieval-hexagon-gameboard/types` |
