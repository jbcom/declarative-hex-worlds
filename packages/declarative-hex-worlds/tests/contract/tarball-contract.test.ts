...
describe('package.json metadata', () => {
  ...
  it('packageManager pins pnpm@9.15.9 at the workspace root', () => {
    // packageManager lives on the workspace-root package.json (pnpm reads it
    // there for the whole workspace), not the published library package.
    const rootPkg = JSON.parse(
      readFileSync(join(findWorkspaceRoot(import.meta.dirname), 'package.json'), 'utf8'),
    ) as PackageJson;
