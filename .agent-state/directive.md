...
- [ ] [WAIT] **E0h** — Sweep remaining src/ files to 100% (paired with E0a per-PR). PR#10 closures advanced many of these in tandem with E0a. Status post-merge:
    ...
    - `src/cli/commands/{blueprint,summarize-plan,snapshot,spawn-groups,patrol-routes}.ts` advanced by a direct-import blueprint-derived command batch; `src/cli/_shared.ts` advanced by guide-filter, authored-option, piece-source-root, and scalar-helper coverage. The larger CLI command surface still has low-coverage command modules.
    Each file's continuation work lands as one commit that adds ≤200 LOC of test code and ratchets the floor.
- [ ] [WAIT] **E8** — Flip coverage thresholds to **100 / 100 / 100 / 100** in `vitest.coverage.shared.ts`. Depends on E0a + E0h reaching the floor at 100. Current PR #109 CI-measured merged coverage is 79.02/73.84/86.49/78.76 with thresholds staged to 78.3/73.0/85.6/78.0; unblocks when the per-file gaps in the E0a/E0h lists close out. Final ratchet commit.
