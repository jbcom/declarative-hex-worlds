...
- [ ] RFC0-8 `./tileset` subpath: tileset manifest, UV-cell math, textured-hex mesh from the coordinate module's honeycomb corners; browser test rendering a small tileset board; SimpleRPG gains a tileset render mode. (Q1 manifest shape + Q2 material: decide with RFC-leaned defaults — separate TilesetManifest, MeshBasicMaterial default.)
  ...
  - REMAINING: (a) `buildTexturedHexMesh` — hex geometry from honeycomb `.corners` (honeycomb-grid `defineHex`, used in src/coordinates/grid.ts) + MeshBasicMaterial with sheet texture + per-cell UV offset from CellRect; new sheet-texture loader/cache in src/three. (b) bridge dispatch: thread optional AssetSource through syncGameboardPlacementObjects, branch on AssetRenderRequest.type ('gltf'→loadGameboardPlacementObject, 'tileset-cell'→buildTexturedHexMesh). (c) browser test rendering a small tileset board. (d) `./tileset` subpath if separate from ./asset-source. THEN the declarative elements (own directive item, effectively RFC0-8b): <HexWorld>/<Tile>/<Tileset>/<Sprite>/<Model> + useHexWorld/useTile/useSelection/useCamera/useHexPath/usePlacement. Design of record: docs/plans/declarative-render-surface.design.md.
