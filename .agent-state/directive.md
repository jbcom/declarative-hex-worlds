...
- [ ] RFC0-CORE `./core` entrypoint (koota-free + three-free) — do FIRST (DEP + RENDER build on the neutral boundary it establishes): SPLIT recipe.ts / coordinates/projection.ts / coordinates/layout.ts into pure + koota modules; barrel the 8-already-pure (asset-source, coordinates, grid, navigation, occupancy, plan, validation, manifest/schema) + newly-split-pure modules into `./core`; contract test asserts `./core` imports pull ZERO koota/three. A `./core` consumer does declarations + JSON + validation + hex math + interop, bring-your-own runtime/renderer.
  - BLAST RADIUS MAPPED (2026-07-06, before code): the koota touchpoints to extract into runtime modules —
    (a) recipe.ts: `applyGameboardRecipeGeneration` (only koota use, line ~664, calls `createGameboardWorld` to run seeded layout-fill then project back). `createGameboardPlanFromRecipe` ALWAYS calls it (line ~438), so even a no-generation recipe statically pulls koota. Fix: move applyGameboardRecipeGeneration to a runtime module; have the pure compiler INJECT the generation-applier (or handle only non-generation recipes in core, delegate generation via a hook). 10 callers of createGameboardPlanFromRecipe (coverage-evidence/{smoke,exercises}, interop, runtime, simulation/script-validators, scenario, catalog-treatments, catalog-guide-data, blueprint, recipe) — move each with the seam.
    (b) projection.ts: `projectWorldToGameboardPlan`, `readDecomposedTileSpecs`, `readValidationGameboardPlanFromWorld` are World-taking (koota) → runtime module; the axialToWorld-based projection helpers are pure → core.
    (c) layout.ts: `spawnGameboardLayoutFill(world)`, `projectWorldForLayout(world)` are koota → runtime; site-inspection / fill-analysis / placement-generation are pure plan-based → core.
    APPROACH: extract World-taking fns into sibling `*-runtime.ts` modules, keep pure fns in the core-eligible file, thread koota-generation via injection, move ALL callers in the same commit (refactors-not-shims). Contract test: import `./core` in a child process with koota UNINSTALLED-simulated (or assert the built `dist/core.js` chunk has zero `koota` refs).
    LOCKED DESIGN (2026-07-06, seam verified): recipe split uses INJECTION with a pure default. `applyGameboardRecipe` (builder steps) is ALREADY koota-free ✓; only `applyGameboardRecipeGeneration` (seeded layout-fill via `createGameboardWorld` → `spawnGameboardLayoutFill` → `projectWorldToGameboardPlan`, line ~664) needs koota. Move it to `src/scenario/recipe-generation.ts` (runtime). `createGameboardPlanFromRecipe` gains an optional `applyGeneration` param: core's pure default returns the built plan when no generation-fill-rules, throws a CLEAR "recipe generation requires the runtime tier" if generation IS present; the runtime tier passes the koota applier by default → all 10 callers keep today's behavior. Same injection pattern for projection.ts (World-readers → runtime) + layout.ts (world spawn/project → runtime). NEXT ITERATION: implement the recipe split first (test → move → inject → move callers → green), then projection, then layout, then the ./core barrel + tsup entry + zero-koota contract test.
