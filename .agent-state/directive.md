...
- [x] **CR-P3-3** — ✅ `interop/coverage.ts` cohesion: documented the release-tooling vs runtime interop split in the architecture page and mirrored the distinction in the `coverage.ts` module comment. Local proof: `pnpm exec vitest run src/interop/__tests__/coverage.test.ts tests/contract/docs-frontmatter-contract.test.ts --config vitest.config.ts`; `pnpm lint`; `pnpm typecheck`; `pnpm docs-site:build` (discarded unrelated generated CLI-reference drift). [AR-7]
