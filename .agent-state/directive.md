...
- [~] RFC0-RENDER 2D/3D fundamental via SIGNALS + BINDINGS (koota-native) — RE-ARCHITECTED (user decision 2026-07, supersedes the RenderBackend seam). The engine emits reactive SIGNALS (koota traits: PlacementState/WorldPosition/etc.) and renderer BINDINGS subscribe + reconcile — exactly koota+react-bindings. Core imports NO renderer. Bindings ship as optional-peer SUBPATHS of the one package (koota-faithful, like `koota/react`): `declarative-hex-worlds/three` (peer: three + @react-three/fiber), later `/pixi`, `/react-dom` — each renderer an OPTIONAL peerDependency. KEPT from prior work (renderer-neutral, still correct): AssetTransform in neutral asset-source (3dd25a1), AssetDimension='2d'|'3d' on every AssetRenderRequest + sources emit it. SUPERSEDED (ab6e19f): the imperative RenderBackend<TNode> interface + createThreeRenderBackend mount/unmount seam → replaced by reactive koota-query reconciliation. TODO: (a) define the signal traits the bindings subscribe to (they mostly EXIST — PlacementState/WorldPosition; audit what a binding needs); (b) refactor src/three from an imperative bridge the core CALLS into a pure signals SUBSCRIBER (bindings: `useQuery(...) → three.Object3D` reconciliation); (c) make three/@react-three/fiber OPTIONAL peers with `peerDependenciesMeta`; (d) contract test tests/contract/renderer-optionality-contract.test.ts — walk each CORE entrypoint's import graph, FAIL CI if it transitively imports three/pixi/R3F (only /three,/pixi,/react-elements may); (e) a 2D binding (pixi or canvas) proves the seam is genuinely substrate-agnostic — that's what makes 2D/3D FUNDAMENTAL not cosmetic. Delete src/asset-source/render-backend.ts + src/three/three-backend.ts (superseded). See [[dhw-signals-bindings-architecture]].

**Phase A — SimpleRPG as first consumer + gap-finder (re-planned around signals+bindings):**
- [ ] RFC0-2 Promote SimpleRPG to `packages/simple-rpg` — a real consumer that renders through a dhw binding SUBPATH (`declarative-hex-worlds/three`). DONE so far (renderer-neutral, survives the pivot): (1/n commit 65e12e3) game module src/game/scenario.ts (loads+validates+compiles via public /scenario+/runtime), fixtures owned by the package, game tests. (2/n commit 04d33ab) layered CI `simple-rpg` job needs:[check]. RE-PLAN (blocked on RFC0-RENDER signals+bindings): src/board.tsx currently uses the /react-elements `<HexWorld>`+`<GameboardObjects>` element surface — that becomes a THREE-BINDING consumer once RFC0-RENDER lands (subscribe to koota signals → three). The e2e migration (the rich public-API harness tests/simple-rpg/simple-rpg.ts + tests/unit/simple-rpg.test.ts, both 100% public API — validation→worldgen→spawn→interop→sim→actor-target→quests) moves INTO the package as its owned e2e; the coverage-evidence guide-API catalog STAYS in the library (done: it's now self-contained, commit 6de7ad7). The visual tests + 5 baselines + third-party-assets test ALSO move to the package rendering through the three BINDING (public API), and src/interop screenshot contract repoints (folds RFC0-4/5/6 forward — user-approved). Sequencing: RFC0-RENDER (signals+bindings + optional peers) FIRST, then this migration against the new binding subpath, then colocate library tests (dissolve the monolithic tests/ tree into src/**/__tests__ — user directive).
...
- [ ] RFC0-CAM Camera/viewport command surface (GAP from SimpleRPG's `viewport` run-state): frame-board, fill-viewport, set perspective/angle (top-down/iso/tilted, ortho vs perspective), orient. Emitted as signals the binding subscribes to (camera is a signal too), + a `useCamera` hook in the three binding. Browser test per mode.
- [ ] RFC0-TESTS Dissolve the library's monolithic `tests/` tree into colocated `src/**/__tests__` (user directive: tests belong as __tests__ where they apply). tests/unit/* → next to their source; decide homes for tests/contract (cross-cutting graph-walkers), tests/browser (library render/visual). Repoint every vitest config's include globs. Do AFTER the SimpleRPG-specific tests move out (RFC0-2). 66 tests already colocated correctly; 34 in the monolith to relocate.
