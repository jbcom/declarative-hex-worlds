...
- [ ] task-LF2 Create src/config/ with logically-decomposed JSON (kaykit-source incl. final git-clone patterns from LF7, bootstrap-paths, upstream-layouts) + typed loader; migrate hardcoded constants from their settled home (post-LF4/LF7) to derive from config; no behavior change.
