...
- [ ] RFC0-PACKS Two tiers. DOWNLOADABLE DEFAULTS = DONE via RFC0-10 (registry + CLI --pack + all three KayKit packs). REMAINING = the BAKED tier: a few Kenney CC0 pieces tracked in packages/examples/assets — Kenney Hexagon Kit→tiles/ (size-norm test exercising RFC0-NORM), Kenney Retro-ish models/ (overlay test exercising RFC0-OVERLAY) — proving cross-maker EXTENSION (a DIFFERENT asset maker than KayKit composes through the same seams). Kenney attribution in NOTICE+docs.
