...
- [ ] [WAIT] **E0a** — Simulation + patrol files toward 100%. PR#10-#51 merged; no open PR remains for the old PR#52 note as of 2026-06-25. Current CI-measured merged coverage gate after browser-free integration is 76.74 / 70.87 / 84.69 / 76.48 (PR #106 Coverage); local reference-enabled runs read 78.19 / 72.18 / 86.11 / 77.94. CLI command batch pending PR/CI confirmation locally passed `CI=true pnpm coverage:all:enforce` at 79.74 / 74.10 / 87.07 / 79.52 and stages thresholds at 79.2 / 73.6 / 86.5 / 79.0. Ratchet against CI-measured merged coverage after each ≤200 LOC test batch; E8's 100/100/100/100 flip still waits on the per-file gaps below. NOTE: scenario.ts:1189 (spawn-group warning map) + 1204 (patrol warning map) effectively unreachable — spawn groups never emit warnings; patrol warning in patrol-route planning needs >1 waypoint + 0 segments (contradictory). Skip these arms.
...
- [ ] [WAIT] **E0h** — Sweep remaining src/ files to 100% (paired with E0a per-PR). PR#10 closures advanced many of these in tandem with E0a. Status post-merge:
    ...
    - `src/cli/commands/{blueprint,summarize-plan,snapshot,spawn-groups,patrol-routes}.ts` advanced by a direct-import blueprint-derived command batch; the larger CLI command surface still has low-coverage command modules and `src/cli/_shared.ts` branches.
    Each file's continuation work lands as one commit that adds ≤200 LOC of test code and ratchets the floor.
- [ ] [WAIT] **E8** — Flip coverage thresholds to **100 / 100 / 100 / 100** in `vitest.coverage.shared.ts`. Depends on E0a + E0h reaching the floor at 100. Current pending CLI-command batch local merged coverage is 79.74/74.10/87.07/79.52 with thresholds staged to 79.2/73.6/86.5/79.0; unblocks when the per-file gaps in the E0a/E0h lists close out. Final ratchet commit.
