...
export function runGuideUsages(parsed: ParsedArgs, sourceRoot: string, edition: PackEdition): void {
  const scenarioFilter = readCsv(parsed.flags.scenarioId ?? parsed.flags.scenario);
  const pageFilter = readGuideScenarioPageFilter(parsed.flags.page);
  const editionFilter = readGuideScenarioEditionFilter(parsed.flags.editionScope);
  const publicApiFilter = readCsv(parsed.flags.publicApi);
...
export function runGuideRenderRequests(
  parsed: ParsedArgs,
  sourceRoot: string,
  edition: PackEdition
): void {
...
export function runGuideRoles(parsed: ParsedArgs): void {
  const roleFilter = readCsv(parsed.flags.role ?? parsed.flags.guideRole);
...
export function formatGuideUsageLine(usage: KayKitGuideScenarioAssetUsage): string {
  return `${usage.label}: ${usage.role}, ${usage.minimumEdition}, ${usage.sourcePath}`;
}

export function formatGuideRenderGroupLine(group: KayKitGuideScenarioAssetRenderGroup): string {
  return `page ${group.page}: ${group.count} render request(s), ${group.scenarioId}`;
}

export function formatGuideRenderRequestLine(
...
export function filterGuideAssetCoverages(
...
export function countGuidePermutationsByKind(
...
export function printManifestInspection(
  path: string,
